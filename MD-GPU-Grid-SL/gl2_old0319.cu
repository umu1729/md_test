...
void CalculateForce_UseGPU(float* h_p,float *h_f,float* h_v,float* d_p,float *d_f,float *d_v,int nElem,float length,WorkBufList wbl){
    ...
    cudaMemcpy(h_f,d_f,nBytes*3,cudaMemcpyDeviceToHost);
    cudaMemcpy(h_p,d_p,nBytes*3,cudaMemcpyDeviceToHost);
    cudaMemcpy(h_v,d_v,nBytes*3,cudaMemcpyDeviceToHost);
...
void cuMain(void (*grpc)(V3Buf buf) ){
    ...
    CalculateForce_UseGPU(h_p,h_f,h_v,d_p,d_f,d_v,nElem,length,wbl);
