...
__global__ void UpdateDifferentialPosition(float *p,float *v,int nElem,float dt,SystemInfo *sysInfo,float s){
...
void cuMain(void (*grpc)(V3Buf buf) ){
    ...
    while(true & it < 20*300){
        ...
        double s_coeff = (1 + ps * dt / Q / 4);
        s *= s_coeff * s_coeff;
        ps /= s_coeff;
        
        printf("#1#s=%lf,ps=%lf,sco=%lf\n",s,ps,s_coeff);
        ...
        ps -= h_sysInfo->sumPotential * dt /2;
        ...
        UpdateDifferentialPosition<<<grid,block>>>(d_p,d_v,nElem,dt,d_sysInfo,s);
        cudaDeviceSynchronize();
        
        double predictH = h_sysInfo->sumPotential+h_sysInfo->sumKinetic;
        //ps += (h_sysInfo->sumKinetic - 3*nElem*T_target*logf(s) + predictH - 3*nElem*T_target)*dt;
        ps -= (h_sysInfo->sumKinetic - 3*nElem*T_target/2)*dt;
