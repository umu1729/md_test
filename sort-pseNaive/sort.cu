...
void sort(int* d_buf,int* d_work,int nLog2Elem){
    int nLogE = nLog2Elem;
    int nElem = 1 << nLogE;
    for(int i=0;i<nLogE;i++){
        ...
        int nSizL = 1<<nLogL;
        for(;j>= nLogL;j--){
                bitonicSort_Global<<<nElem/256,256>>>(d_buf,d_work,i,j);
                SwapIntPointer(&d_buf,&d_work);
                cudaDeviceSynchronize();
        }
        
        CHECK(cudaGetLastError());
        
        bitonicSort_Local<<<(nElem+nSizL-1)/nSizL,nSizL,2*nSizL*sizeof(int)>>>(d_buf,d_work,i,j);
        SwapIntPointer(&d_buf,&d_work);
        cudaDeviceSynchronize();
